(ns user
  ...
  (:require
    ...
    [dumch.manipula.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (dumch.manipula.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (dumch.manipula.config/system-config {:profile :test})
                                  (ig/prep)))))
...
(comment
  ...
  (require '[flow-storm.api :refer [local-connect stop]])
  (local-connect)
  (stop)
