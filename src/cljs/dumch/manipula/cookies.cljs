(ns dumch.manipula.cookies
  (:refer-clojure :exclude [count get keys vals empty? reset!])
  (:require
    [cljs.reader :as reader]
    [goog.net.Cookies :refer [SetOptions]]))

(defonce ^:dynamic ^goog.net.Cookies  *cookies*
  (or goog.net.cookies (.getInstance goog.net.Cookies)))
...
(defn set!
  ...
  (let [k (name k)
        content (if raw?
                  (js/encodeURIComponent (str content))
                  (pr-str content))]
    (cond
